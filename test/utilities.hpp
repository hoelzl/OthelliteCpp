...
namespace reviser {
    ...
    struct NotifierSpy final : public Notifier {
        ...
        struct Move {
            Move(std::string player_name, PlayerColor color, int row, int column)
                    : player_name(std::move(player_name)), color(color), row(row), column(column) {}
    ...
    inline bool
    operator==(const NotifierSpy::Move &lhs, const NotifierSpy::Move &rhs) {
        return lhs.player_name == rhs.player_name && lhs.color == rhs.color
               && lhs.row == rhs.row && lhs.column == rhs.column;
    }

    inline std::ostream &operator<<(std::ostream &os, const NotifierSpy &spy) {
